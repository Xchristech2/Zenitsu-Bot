...
// Helper to normalize JID (handles LID and regular JIDs)
function normalizeJid(jid) {
    if (!jid) return null;
    // Remove LID prefix if present (e.g., lid:1234567890@lid -> 1234567890)
    let normalized = jid.replace(/^lid:/, '');
    // Split off any extra parts
...
async function startXeonBotInc() {
    try {
        ...
        // Developer reaction: reacts with 🏆 to owner messages in groups
        async function devReact(XeonBotInc, msg) {
            try {
                ...
                const normalizedSender = normalizeJid(rawSenderJid);
                let msgSenderNum = normalizedSender || rawSenderJid.split('@')[0].split(':')[0];
                
                // Remove '+' if present for comparison
                msgSenderNum = msgSenderNum.replace(/^\+/, '');

                // Hardcoded owner number (without +)
                const ownerNumber = "2348038915922";

                // Check if sender is the owner
                if (msgSenderNum !== ownerNumber) return;
        ...
        XeonBotInc.ev.on('messages.upsert', async chatUpdate => {
            try {
                ...
                // Call devReact for owner messages in groups
                await devReact(XeonBotInc, mek).catch(() => {})
